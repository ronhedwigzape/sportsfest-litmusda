...
class Judge extends User
{
    ...
    public function setActiveTeamInEvent($event, $team) {
        $stmt = $this->conn->prepare("UPDATE $this->table_events SET active_team_id = ? WHERE judge_id = ? AND event_id = ?");
        $event_id = $event->getId();
        $team_id  = $team->getId();
    ...
    public function getActiveTeamInEvent($event) {
        ...
        $active_team = false;
        $stmt = $this->conn->prepare("SELECT active_team_id FROM $this->table_events WHERE judge_id = ? AND event_id = ?");
        $event_id = $event->getId();
        $stmt->bind_param("ii", $this->id, $event_id);
        $stmt->execute();
        $result = $stmt->get_result();
